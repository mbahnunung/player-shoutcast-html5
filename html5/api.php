		...
		if(!empty($data)) {
			...
			if(count($currentSong) === 2) {
				if(substr($currentSong[1], -14) === '</body></html>') {
					$array['currentSong'] = (!empty($currentSong[1])) ? trim(substr($currentSong[1], 0, -14)) : 'No Title';
				} else {
					$array['currentSong'] = (!empty($currentSong[1])) ? $currentSong[1] : 'No Title';
				}

				$array['currentArtist'] = (empty($currentSong[1])) ? trim(substr($playingNow, 0, -14)) : $artist;
			} elseif(count($currentSong) < 2) {
				$array['currentSong'] = (empty($playingNow) || $playingNow === '</body></html>') ? 'No Title' : trim(substr($playingNow, 0, -14));
				$array['currentArtist'] = 'No Artist';
			} else {
